...
const ChatPage = () => {
  ...
  const mutate = useChatAi({
    onSuccess: (data) => {
      ...
      addMessage({
        sender: "ai",
        text: text.data || "Xin lỗi, tôi không hiểu câu hỏi của bạn",
      });
    },
    onError: () => {
      setLoading(false);
        ...
        {messages.length === 0 ? (
          ...
          <div>
            {messages.map((item, index) => (
              ...
              >
                <div
                  className={`flex items-start ${item.sender === "ai" ? "flex-row" : "flex-row-reverse"
                    }`}
                >
                  <Avatar
                    icon={
                  ...
                  />
                  <Markdown
                    className={`p-2 rounded ${item.sender === "user" ? "mr-2" : "ml-2"
