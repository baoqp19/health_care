...
// Định nghĩa kiểu cho một note
interface Note {
  user_id?: number; // Optional vì không phải tất cả các ghi chú đều có user_id
  index: number;
  title: string;
  content: string;
  created_at: string;
...
const NotePage = () => {
  ...
  const handleOk = (values: { title: string; content: string }) => {
    if (isEditMode && currentNote) {
      setNotes(
        notes.map(note =>
          note.index === currentNote.index ? { ...currentNote, ...values } : note
        )
      );
    } else {
